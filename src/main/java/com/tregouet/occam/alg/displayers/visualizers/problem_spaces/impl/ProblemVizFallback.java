...
public class ProblemVizFallback implements ProblemSpaceViz {

	public static final ProblemVizFallback INSTANCE = new ProblemVizFallback();

	private ProblemVizFallback() {
	}
	...
	public String apply(DirectedAcyclicGraph<IRepresentation, AProblemStateTransition> graph, String fileName) {
		...
		try {
			...
			DOTExporter<IRepresentation, AProblemStateTransition> exporter = new DOTExporter<>();
			ProblemStateLabeller stateDisplayer = new AsNestedFrames();
			ProblemTransitionLabeller transitionDisplayer = NoLabel.INSTANCE;
			exporter.setVertexAttributeProvider((v) -> {
				Map<String, Attribute> map = new LinkedHashMap<>();
				map.put("label", DefaultAttribute.createAttribute(stateDisplayer.apply(v)));
				return map;
