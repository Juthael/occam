package com.tregouet.occam.alg.scorers.difference_DEP.impl;
...
public class TwoLeafTreeDEP implements DifferenceScorerDEP {

	public static final TwoLeafTreeDEP INSTANCE = new TwoLeafTreeDEP();

	private TwoLeafTreeDEP() {
	}
	...
	public double score(int particularID1, int particularID2, IConceptLattice lattice) {
		...
		IClassification classification =
				DifferenceScorerDEP.classificationBuilder().apply(twoLeafTree, lattice.getParticularID2Particular());
		Set<IContextualizedProduction> productions = DifferenceScorerDEP.productionSetBuilder().apply(classification.normalized());
		IDescription description = DifferenceScorerDEP.descriptionBuilder().apply(classification.normalized(), productions);
